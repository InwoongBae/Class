...
class Cube
{
  ...
  public:
  Cube(const vector<string>& tokens) : side(tokens.size() >= 2 ? atof(tokens[1].c_str()) : 0){};
...
class Box
{
  ...
  public:
  Box(const vector<string>& tokens) : length(tokens.size() >= 2 ? atof(tokens[1].c_str()) : 0), width(tokens.size() >= 3 ? atof(tokens[2].c_str()) : 0), height(tokens.size() >= 4 ? atof(tokens[3].c_str()) : 0){};
...
class Cylinder
{
  ...
  public:
  Cylinder(vector<string> tokens) : radius(tokens.size() >= 2 ? atof(tokens[1].c_str()) : 0), height(tokens.size() >= 3 ? atof(tokens[2].c_str()) : 0){};
...
class Prism
{
  ...
  public:
  Prism(vector<string> tokens) : side(tokens.size() >= 2 ? atof(tokens[1].c_str()) : 0), height(tokens.size() >= 2 ? atof(tokens[1].c_str()) : 0){};
...
int main()
{
  ...
  {
    for (unsigned int j = i + 1; j < myBag.size(); j++)
    {
